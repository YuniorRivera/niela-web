  ...
  useEffect(() => {
    import('canvas-confetti').then(({ default: confetti }) => {
      confetti({
        ...
        colors: ['#d4e3d4', '#e8d9b5', '#a8c9a8', '#ffffff', '#D4A857'],
      })
      setTimeout(() => {
  ...
  return (
    <div style={{ textAlign: 'center', padding: '8px 0' }}>
      ...
      <motion.p
        ...
        style={{ fontSize: 20, fontWeight: 700, color: '#ffffff', margin: '0 0 6px' }}
      >
        {t('title')}
      </motion.p>
      <motion.p
        ...
        style={{ fontSize: 14, color: '#D4A857', margin: '0 0 6px', fontWeight: 500 }}
      >
        {t('position', { position })}
      </motion.p>
      <motion.p
        ...
        style={{ fontSize: 13, color: 'rgba(255,255,255,0.6)', margin: '0 0 20px' }}
      >
        {t('subtitle')}
      </motion.p>
      <motion.p
        ...
        style={{ fontSize: 12, color: 'rgba(255,255,255,0.45)', margin: '0 0 14px' }}
      >
      ...
      >
        ...
        <button onClick={copyLink} style={{ padding: '10px 16px', borderRadius: 12, background: 'rgba(212,168,87,0.15)', color: '#D4A857', fontSize: 13, fontWeight: 600, border: '1px solid rgba(212,168,87,0.3)', cursor: 'pointer' }}>
...
function WaitlistForm({ dark = false, onSuccess }: { dark?: boolean; onSuccess?: (position: number) => void }) {
  ...
  const inputBg = dark ? 'rgba(255,255,255,0.08)' : 'rgba(255,255,255,0.7)'
  const inputBorder = dark ? '1px solid rgba(255,255,255,0.18)' : '1px solid rgba(44,62,80,0.18)'
  const inputColor = dark ? '#e8f1f5' : '#2c3e50'
  const btnBg = dark ? '#e8f1f5' : '#D4A857'
  const btnColor = dark ? '#1f2d3a' : '#1f2d3a'
  const legalColor = dark ? 'rgba(255,255,255,0.5)' : '#6b7c8a'
  const linkColor = dark ? '#D4A857' : '#4a6b80'
  ...
  return (
    <div style={{ width: '100%', maxWidth: 460 }}>
      ...
      <label style={{ display: 'flex', alignItems: 'flex-start', gap: 8, cursor: 'pointer', marginBottom: 8 }}>
        <input type="checkbox" checked={accepted} onChange={e => setAccepted(e.target.checked)} style={{ marginTop: 2, accentColor: '#D4A857', flexShrink: 0 }} />
...
export default function Home() {
  ...
  const [secPricingRef, secPricingIn] = useInView(0.1)
    ...
    <main style={{ margin: 0, minHeight: '100vh', background: '#0A0E14', overflowX: 'hidden' }}>
      ...
      <nav style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '24px 48px', position: 'fixed', top: 0, left: 0, right: 0, zIndex: 100, background: '#0A0E14', backdropFilter: 'blur(16px)', borderBottom: '0.5px solid rgba(255,255,255,0.08)' }}>
        ...
        <div style={{ display: 'flex', gap: 32, alignItems: 'center' }}>
          <a href="#como-funciona" style={{ fontSize: 14, color: 'rgba(255,255,255,0.65)', textDecoration: 'none', transition: 'color 200ms' }}>{t('nav.comoFunciona')}</a>
          <a href="#tradiciones" style={{ fontSize: 14, color: 'rgba(255,255,255,0.65)', textDecoration: 'none', transition: 'color 200ms' }}>{t('nav.tradiciones')}</a>
          <a href="#comparativa" style={{ fontSize: 14, color: 'rgba(255,255,255,0.65)', textDecoration: 'none', transition: 'color 200ms' }}>{t('nav.comparativa')}</a>
          <div className="lang-switcher-wrap"><LangSwitcher /></div>
          <a href={PLAY_STORE_URL} target="_blank" rel="noopener" style={{ background: '#D4A857', color: '#1f2d3a', border: 'none', padding: '10px 20px', borderRadius: 999, fontSize: 13, fontWeight: 600, textDecoration: 'none', transition: 'transform 150ms, opacity 150ms' }}>{t('nav.download')}</a>
        </div>
      </nav>

      {/* ══ SECTION 1: HERO ══ */}
      <section className="hero-section" style={{ minHeight: '100vh', background: 'linear-gradient(160deg, #0A0E14 0%, #0c1420 60%, #0A0E14 100%)', position: 'relative', overflow: 'hidden', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 40, alignItems: 'center', padding: '88px 64px 80px', maxWidth: 1240, margin: '0 auto' }}>
        {/* Decorative blur spheres */}
        <div style={{ position: 'absolute', top: '15%', right: '5%', width: 420, height: 420, borderRadius: '50%', background: 'radial-gradient(circle, rgba(212,168,87,0.18) 0%, rgba(212,168,87,0) 70%)', pointerEvents: 'none' }} />
        ...
        <div style={{ position: 'relative', zIndex: 5 }}>
          {/* Users chip */}
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '7px 16px', background: 'rgba(212,168,87,0.12)', border: '1px solid rgba(212,168,87,0.25)', borderRadius: 999, fontSize: 13, color: '#D4A857', marginBottom: 28, backdropFilter: 'blur(12px)', fontWeight: 500 }}>
            {count != null && count > 0 ? t('hero.usersChip', { count: count.toLocaleString('es') }) : t('hero.usersChipFallback')}
          </div>

          {/* Headline */}
          <h1 className="hero-title" style={{ fontSize: 72, lineHeight: 1.0, fontWeight: 500, color: '#ffffff', margin: '0 0 24px', letterSpacing: '-2.5px' }}>
            ...
            {t('hero.title3') && (
              <span className={`hero-line hero-line-3${heroMounted ? ' hero-line-visible' : ''}`} style={{ display: 'block', color: '#D4A857', fontStyle: 'italic' }}>{t('hero.title3')}</span>
            )}
          </h1>

          <p style={{ fontSize: 17, lineHeight: 1.65, color: 'rgba(255,255,255,0.72)', margin: '0 0 32px', maxWidth: 440 }}>
            {t('hero.subtitle')}
          </p>
          ...
          <div style={{ display: 'flex', gap: 14, marginBottom: 20, flexWrap: 'wrap', alignItems: 'center' }}>
            <button
              ...
              style={{ background: '#C8A96E', color: '#0A0A0F', border: 'none', padding: '14px 28px', borderRadius: 999, fontSize: 15, fontWeight: 600, cursor: 'pointer', letterSpacing: '-0.2px', transition: 'transform 150ms, opacity 150ms' }}
          ...
          <p style={{ color: 'rgba(255,255,255,0.35)', fontSize: 12, marginBottom: 28, marginTop: 0 }}>Sin cargo hasta el día 14 · Cancela cuando quieras</p>
        ...
        <div className="hero-mockup" style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', position: 'relative' }}>
          ...
          <div className="float-anim" style={{ position: 'relative', zIndex: 5, width: 290, height: 580, background: '#060810', borderRadius: 44, padding: 10, boxShadow: '0 40px 80px rgba(0,0,0,0.45), 0 12px 32px rgba(0,0,0,0.3)' }}>
      ...
      <section
        ...
        style={{ padding: '48px 64px', background: 'rgba(255,255,255,0.03)', borderTop: '1px solid rgba(212,168,87,0.08)', borderBottom: '1px solid rgba(212,168,87,0.08)' }}
      >
          ...
          <div style={{ display: 'flex', gap: 0, justifyContent: 'center', alignItems: 'center', flexWrap: 'wrap', marginBottom: 24 }}>
            {[t('trustBar.evidence'), t('trustBar.gdpr'), t('trustBar.encryption'), t('trustBar.advisory')].map((item, i, arr) => (
              <span key={item} style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                <span style={{ fontSize: 14, color: 'rgba(255,255,255,0.65)', fontWeight: 400, letterSpacing: '0.2px' }}>{item}</span>
                {i < arr.length - 1 && <span style={{ color: 'rgba(212,168,87,0.3)', margin: '0 16px', fontSize: 12 }}>·</span>}
              </span>
            ))}
          </div>
        </div>
      ...
      <section
        ...
        style={{ padding: '100px 64px', background: '#0A0E14', position: 'relative' }}
      >
        <div style={{ textAlign: 'center', marginBottom: 56, maxWidth: 720, margin: '0 auto 56px' }}>
          <h2 className={`fade-up${secTradIn ? ' in-view' : ''}`} style={{ fontSize: 44, fontWeight: 500, color: '#ffffff', margin: '0 0 16px', letterSpacing: '-1px' }}>
            {t('traditions2.title')}
          </h2>
          <p className={`fade-up stagger-2${secTradIn ? ' in-view' : ''}`} style={{ fontSize: 16, lineHeight: 1.65, color: 'rgba(255,255,255,0.6)', margin: 0 }}>
      ...
      <section
        ...
        style={{ padding: '100px 64px', background: '#0d1118', position: 'relative' }}
      >
        <div style={{ textAlign: 'center', marginBottom: 72, maxWidth: 720, margin: '0 auto 72px' }}>
          <p style={{ fontSize: 12, color: '#D4A857', textTransform: 'uppercase', letterSpacing: '3px', margin: '0 0 16px', fontWeight: 600 }}>{t('howItWorks2.eyebrow')}</p>
          <h2 className={`fade-up${secHowIn ? ' in-view' : ''}`} style={{ fontSize: 44, fontWeight: 500, color: '#ffffff', margin: 0, letterSpacing: '-1px' }}>
            {t('howItWorks2.title')}
          </h2>
        </div>
        ...
        <div className="roadmap-grid" style={{ maxWidth: 900, margin: '0 auto' }}>
          {[
            ...
            >
              <div style={{ fontSize: 80, fontWeight: 300, color: 'rgba(212,168,87,0.12)', lineHeight: 1, margin: '0 0 16px', fontVariantNumeric: 'tabular-nums' }}>{step.num}</div>
              <h3 style={{ fontSize: 20, fontWeight: 600, color: '#ffffff', margin: '0 0 12px' }}>{step.title}</h3>
              <p style={{ fontSize: 15, lineHeight: 1.65, color: 'rgba(255,255,255,0.6)', margin: 0 }}>{step.desc}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      >
        <div style={{ textAlign: 'center', marginBottom: 64, maxWidth: 720, margin: '0 auto 64px' }}>
          <p style={{ fontSize: 12, color: '#D4A857', textTransform: 'uppercase', letterSpacing: '3px', margin: '0 0 16px', fontWeight: 600 }}>{t('roadmap.eyebrow')}</p>
          <h2 className={`fade-up${secRoadIn ? ' in-view' : ''}`} style={{ fontSize: 44, fontWeight: 500, color: '#ffffff', margin: 0, letterSpacing: '-1px' }}>
            {t('roadmap.title')}
          </h2>
        </div>
        ...
        <div className="roadmap-grid" style={{ maxWidth: 1100, margin: '0 auto 64px' }}>
          {[
            ...
            <div
              ...
              style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(212,168,87,0.1)', borderRadius: 20, padding: '32px 28px' }}
            >
              <p style={{ fontSize: 13, fontWeight: 500, color: '#D4A857', textTransform: 'uppercase', letterSpacing: '2px', margin: '0 0 20px' }}>
                {ci === 0 ? '✅ ' : ''}{col.title}
              </p>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                {col.items.map((item, ii) => (
                  <div key={ii} style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                    ...
                    <span style={{ fontSize: 15, color: col.done[ii] ? 'rgba(255,255,255,0.85)' : 'rgba(255,255,255,0.5)', lineHeight: 1.45 }}>{item}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div style={{ textAlign: 'center', maxWidth: 560, margin: '0 auto' }}>
          <p style={{ fontSize: 16, color: 'rgba(255,255,255,0.65)', margin: '0 0 24px', lineHeight: 1.6 }}>{t('roadmap.emailCta')}</p>
      ...
      <section
        ...
        style={{ padding: '100px 64px', background: '#0d1118' }}
      >
        <div style={{ textAlign: 'center', marginBottom: 64 }}>
          <p style={{ fontSize: 12, color: '#D4A857', textTransform: 'uppercase', letterSpacing: '3px', margin: '0 0 16px', fontWeight: 600 }}>{t('comparison.eyebrow')}</p>
          <h2 className={`fade-up${sec6In ? ' in-view' : ''}`} style={{ fontSize: 44, fontWeight: 500, color: '#ffffff', margin: 0, letterSpacing: '-1px' }}>{t('comparison.title')}</h2>
        </div>
        <div className="comparativa-wrap" style={{ maxWidth: 720, margin: '0 auto' }}>
          <div className={`fade-up${sec6In ? ' in-view' : ''}`} style={{ minWidth: 480, borderRadius: 20, overflow: 'hidden', border: '1px solid rgba(212,168,87,0.12)' }}>
            {/* Header */}
            <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr', background: 'rgba(212,168,87,0.06)', borderBottom: '1px solid rgba(212,168,87,0.1)' }}>
              {[t('comparison.headerFeature'), t('comparison.headerNiela'), t('comparison.headerCalm'), t('comparison.headerHeadspace')].map((h, i) => (
                <div key={h} style={{ padding: '18px 24px', fontSize: 13, fontWeight: 600, color: i === 1 ? '#D4A857' : 'rgba(255,255,255,0.5)', textAlign: i === 0 ? 'left' : 'center', background: i === 1 ? 'rgba(212,168,87,0.06)' : 'transparent' }}>{h}</div>
              ))}
            </div>
            {(() => {
              const cellStyle = (ci: number, last: boolean) => ({
                ...
                color: ci === 1 ? '#b8c9a8' : ci === 0 ? 'rgba(255,255,255,0.75)' : 'rgba(255,255,255,0.35)',
                fontWeight: ci === 1 ? 600 : 400,
                background: ci === 1 ? 'rgba(212,168,87,0.04)' : 'transparent',
                borderBottom: last ? 'none' : '1px solid rgba(212,168,87,0.06)',
              });
              const partial = (
                <span style={{ fontSize: 13, color: 'rgba(255,255,255,0.4)', fontWeight: 400 }}>{t('comparison.partial')}</span>
              );
              const rows: { label: string; niela: React.ReactNode; calm: React.ReactNode; headspace: React.ReactNode }[] = [
                ...
                { label: t('comparison.rowAi'), niela: '✓', calm: '✗', headspace: (
                  <span style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
                    <span>✓</span>
                    <span style={{ fontSize: 10, color: 'rgba(255,255,255,0.3)', fontWeight: 400 }}>{t('comparison.ebbNote')}</span>
                  </span>
                )},
        ...
        <p style={{ textAlign: 'center', fontSize: 11, color: 'rgba(255,255,255,0.25)', marginTop: 20, fontStyle: 'italic', maxWidth: 720, margin: '20px auto 0' }}>
          {t('comparison.footer')}
        </p>
      </section>
      ...
      <section
        id="pricing"
        ref={secPricingRef as React.RefObject<HTMLElement>}
        style={{ padding: '100px 64px', background: '#0A0E14', position: 'relative', overflow: 'hidden' }}
      >
        {/* Subtle gold glow top-right */}
        <div style={{ position: 'absolute', top: '-80px', right: '-80px', width: 480, height: 480, borderRadius: '50%', background: 'radial-gradient(circle, rgba(212,168,87,0.10) 0%, rgba(212,168,87,0) 70%)', pointerEvents: 'none' }} />

        <div style={{ textAlign: 'center', marginBottom: 64, maxWidth: 720, margin: '0 auto 64px' }}>
          <p style={{ fontSize: 11, color: '#D4A857', textTransform: 'uppercase', letterSpacing: '3px', margin: '0 0 16px', fontWeight: 500 }}>{t('pricing.eyebrow')}</p>
          <h2 className={`fade-up${secPricingIn ? ' in-view' : ''}`} style={{ fontSize: 44, fontWeight: 400, color: '#ffffff', margin: 0, letterSpacing: '-1px' }}>
            {t('pricing.title')}
          </h2>
        </div>
        ...
        >
          ...
          <div style={{
            ...
            <p style={{ fontSize: 13, fontWeight: 500, color: 'rgba(255,255,255,0.55)', margin: '0 0 8px', letterSpacing: '0.5px' }}>{t('pricing.plusTitle')}</p>
            {/* Monthly price */}
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 32, padding: '10px 14px', background: 'rgba(212,168,87,0.08)', border: '0.5px solid rgba(212,168,87,0.20)', borderRadius: 10 }}>
              ...
              <span style={{ fontSize: 14, color: '#ffffff', fontWeight: 400 }}>{t('pricing.plusYearly')}<span style={{ color: 'rgba(255,255,255,0.4)', fontWeight: 300 }}>{t('pricing.plusPerYear')}</span></span>
              <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.45)', fontWeight: 300 }}>{t('pricing.plusBilledAs')}</span>
              <span style={{ fontSize: 11, color: '#D4A857', fontWeight: 500, marginLeft: 'auto' }}>{t('pricing.plusSaving')}</span>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 32 }}>
              {(['plusF1','plusF2','plusF3','plusF4'] as const).map(k => (
            ...
            <a href={`https://play.google.com/store/apps/details?id=com.niela.app`} target="_blank" rel="noopener" style={{ display: 'block', textAlign: 'center', padding: '14px 0', borderRadius: 999, background: 'transparent', border: '0.5px solid rgba(255,255,255,0.25)', color: '#ffffff', fontSize: 14, fontWeight: 500, textDecoration: 'none', marginTop: 'auto', transition: 'background 200ms' }}>
              {t('pricing.cta')}
            </a>
          </div>
      ...
      <section
        ...
        style={{ padding: '100px 64px', background: '#0A0E14' }}
      >
        <div style={{ maxWidth: 900, margin: '0 auto' }}>
          <p style={{ fontSize: 12, color: '#D4A857', textTransform: 'uppercase', letterSpacing: '3px', margin: '0 0 48px', fontWeight: 600, textAlign: 'center' }}>{t('founder.eyebrow')}</p>
          <div className={`founder-grid fade-up${secFounderIn ? ' in-view' : ''}`}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16 }}>
              ...
              <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.45)', margin: 0, textAlign: 'center' }}>Yunior Rivera</p>
            </div>

            {/* Text */}
            <div>
              <h2 style={{ fontSize: 36, fontWeight: 500, color: '#ffffff', margin: '0 0 24px', letterSpacing: '-0.5px' }}>
                {t('founder.title')}
              </h2>
              <p style={{ fontSize: 16, lineHeight: 1.75, color: 'rgba(255,255,255,0.7)', margin: '0 0 16px' }}>{t('founder.text1')}</p>
              <p style={{ fontSize: 16, lineHeight: 1.75, color: 'rgba(255,255,255,0.7)', margin: '0 0 16px' }}>{t('founder.text2')}</p>
              <p style={{ fontSize: 16, lineHeight: 1.75, color: 'rgba(255,255,255,0.7)', margin: '0 0 28px' }}>{t('founder.text3')}</p>
              <a href="mailto:hola@niela.app" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '12px 24px', background: 'rgba(212,168,87,0.1)', border: '1px solid rgba(212,168,87,0.25)', borderRadius: 12, color: '#D4A857', fontSize: 14, fontWeight: 600, textDecoration: 'none', transition: 'background 200ms' }}>
                ✉ {t('founder.cta')}
              </a>
            </div>
          </div>
      ...
      >
        <div style={{ maxWidth: 720, margin: '0 auto' }}>
          <p style={{ fontSize: 12, color: '#D4A857', textTransform: 'uppercase', letterSpacing: '3px', margin: '0 0 16px', fontWeight: 600, textAlign: 'center' }}>{t('faq.eyebrow')}</p>
          <h2 className={`fade-up${secFaqIn ? ' in-view' : ''}`} style={{ fontSize: 44, fontWeight: 500, color: '#ffffff', margin: '0 0 56px', letterSpacing: '-1px', textAlign: 'center' }}>
            {t('faq.title')}
          </h2>
      ...
      {/* ══ FOOTER RICO ══ */}
      <footer style={{ background: '#060810', borderTop: '1px solid rgba(212,168,87,0.07)', padding: '64px 64px 32px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 40, maxWidth: 1040, margin: '0 auto 48px' }}>

          {/* Col 1: Tradiciones */}
          <div>
            <p style={{ fontSize: 12, fontWeight: 600, color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', letterSpacing: '2px', margin: '0 0 16px' }}>{t('footer.traditionsHeader')}</p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {[
                ...
                <a key={label} href={href} style={{ fontSize: 14, color: 'rgba(255,255,255,0.55)', textDecoration: 'none', transition: 'color 200ms' }}>{label}</a>
              ))}
            </div>
          </div>
          ...
          {/* Col 2: Recursos */}
          <div>
            <p style={{ fontSize: 12, fontWeight: 600, color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', letterSpacing: '2px', margin: '0 0 16px' }}>{t('footer.resourcesHeader')}</p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {[
                ...
                <a key={label} href={href} style={{ fontSize: 14, color: 'rgba(255,255,255,0.3)', textDecoration: 'none', cursor: 'default' }}>{label}</a>
              ))}
            </div>
          </div>
          ...
          {/* Col 3: Empresa */}
          <div>
            <p style={{ fontSize: 12, fontWeight: 600, color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', letterSpacing: '2px', margin: '0 0 16px' }}>{t('footer.companyHeader')}</p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {[
          ...
          {/* Col 4: Descargar */}
          <div>
            <p style={{ fontSize: 12, fontWeight: 600, color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', letterSpacing: '2px', margin: '0 0 16px' }}>{t('footer.downloadHeader')}</p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <a href={PLAY_STORE_URL} target="_blank" rel="noopener" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '10px 16px', background: 'rgba(212,168,87,0.1)', border: '1px solid rgba(212,168,87,0.2)', borderRadius: 10, color: '#D4A857', fontSize: 14, fontWeight: 600, textDecoration: 'none', width: 'fit-content' }}>
              ...
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '10px 16px', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(212,168,87,0.08)', borderRadius: 10, color: 'rgba(255,255,255,0.25)', fontSize: 14, width: 'fit-content', cursor: 'not-allowed' }}>
              ...
              <div style={{ display: 'flex', gap: 12, marginTop: 8 }}>
                {[
                  ...
                  <a key={label} href={href} title={label} style={{ fontSize: 18, color: 'rgba(255,255,255,0.35)', textDecoration: 'none', transition: 'color 200ms' }}>{icon}</a>
                ))}
              </div>
            </div>
          </div>
        ...
        {/* Bottom bar */}
        <div style={{ maxWidth: 1040, margin: '0 auto', paddingTop: 28, borderTop: '1px solid rgba(212,168,87,0.07)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 16 }}>
          <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.35)', margin: 0 }}>{t('footer.copyright')}</p>
      ...
      {stickyVisible && (
        <div className="sticky-mobile-cta" style={{
          ...
          <span style={{ fontSize: 13, color: 'rgba(255,255,255,0.6)' }}>{t('stickyBar.downloadText')}</span>
          <a href={PLAY_STORE_URL} target="_blank" rel="noopener" style={{ background: '#D4A857', color: '#1f2d3a', border: 'none', borderRadius: 999, padding: '10px 20px', fontWeight: 700, fontSize: 14, textDecoration: 'none' }}>
            {t('nav.download')}
          </a>
        </div>
      )}
      ...
      {exitModalOpen && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(8px)', zIndex: 300, display: 'flex', alignItems: 'center', justifyContent: 'center' }}
          ...
          <div onClick={e => e.stopPropagation()} style={{ background: '#2c3e50', borderRadius: 24, padding: '40px 36px', maxWidth: 440, width: '90%', position: 'relative', border: '1px solid rgba(212,168,87,0.15)' }}>
            ...
            <h3 style={{ fontSize: 24, fontWeight: 600, color: '#ffffff', margin: '0 0 8px' }}>{t('exitModal.iosTitle')}</h3>
            <p style={{ fontSize: 15, color: 'rgba(255,255,255,0.65)', margin: '0 0 24px' }}>{t('exitModal.iosSubtitle')}</p>
            {!exitSubmitted ? (
              <form onSubmit={handleExitSubmit}>
                <input type="email" value={exitEmail} onChange={e => setExitEmail(e.target.value)} placeholder={t('hero.emailPlaceholder')} required style={{ width: '100%', padding: '12px 16px', borderRadius: 12, border: '1px solid rgba(212,168,87,0.3)', background: 'rgba(255,255,255,0.06)', color: '#ffffff', fontSize: 15, marginBottom: 12, boxSizing: 'border-box', outline: 'none' }} />
                <button type="submit" disabled={exitLoading} style={{ width: '100%', padding: '14px', borderRadius: 12, border: 'none', background: '#D4A857', color: '#1f2d3a', fontWeight: 700, fontSize: 15, cursor: exitLoading ? 'not-allowed' : 'pointer', opacity: exitLoading ? 0.6 : 1 }}>
      ...
      {/* ══ TRIAL DOWNLOAD MODAL ══ */}
      {trialModalOpen && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', backdropFilter: 'blur(10px)', zIndex: 300, display: 'flex', alignItems: 'center', justifyContent: 'center' }}
          onClick={() => setTrialModalOpen(false)}>
          <div onClick={e => e.stopPropagation()} style={{ background: '#0D1117', borderRadius: 24, padding: '40px 36px', maxWidth: 420, width: '90%', position: 'relative', border: '0.5px solid rgba(200,169,110,0.25)', textAlign: 'center' }}>
            <button onClick={() => setTrialModalOpen(false)} style={{ position: 'absolute', top: 14, right: 14, background: 'none', border: 'none', color: 'rgba(255,255,255,0.35)', fontSize: 20, cursor: 'pointer', lineHeight: 1 }}>✕</button>
            {/* Gota icon */}
            <svg width="40" height="40" viewBox="0 0 120 120" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ marginBottom: 20 }}>
              <ellipse cx="60" cy="78" rx="46" ry="6" stroke="#C8A96E" strokeWidth="1.6" fill="none" opacity="0.3"/>
              <ellipse cx="60" cy="78" rx="36" ry="5" stroke="#C8A96E" strokeWidth="1.8" fill="none" opacity="0.5"/>
              <ellipse cx="60" cy="78" rx="26" ry="4" stroke="#C8A96E" strokeWidth="2" fill="none" opacity="0.7"/>
              <ellipse cx="60" cy="78" rx="16" ry="3" stroke="#C8A96E" strokeWidth="2.2" fill="none" opacity="0.9"/>
              <path d="M60 18 C60 18 50 35 47 50 C45 60 50 70 60 70 C70 70 75 60 73 50 C70 35 60 18 60 18 Z" fill="#C8A96E"/>
            </svg>
            <h3 style={{ fontSize: 22, fontWeight: 500, color: '#ffffff', margin: '0 0 10px', letterSpacing: '-0.3px' }}>Descarga Niela para empezar tu prueba gratuita</h3>
            <p style={{ fontSize: 14, color: 'rgba(255,255,255,0.45)', margin: '0 0 28px', lineHeight: 1.5 }}>14 días gratis · Sin cargo hasta el día 14 · Cancela cuando quieras</p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              <a href={PLAY_STORE_URL} target="_blank" rel="noopener" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, background: '#C8A96E', color: '#0A0A0F', borderRadius: 14, padding: '14px 20px', fontSize: 15, fontWeight: 600, textDecoration: 'none' }}>
                <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M3 20.5v-17c0-.83 1.01-1.3 1.7-.76l14 8.5c.62.38.62 1.14 0 1.52l-14 8.5c-.69.54-1.7.07-1.7-.76z"/></svg>
                Descargar en Google Play
              </a>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, background: 'rgba(255,255,255,0.05)', color: 'rgba(255,255,255,0.35)', borderRadius: 14, padding: '14px 20px', fontSize: 15, fontWeight: 500, border: '0.5px solid rgba(255,255,255,0.08)' }}>
                <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" style={{ opacity: 0.4 }}><path d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.8-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M13 3.5c.73-.83 1.94-1.46 2.94-1.5.13 1.17-.34 2.35-1.04 3.19-.69.85-1.83 1.51-2.95 1.42-.15-1.15.41-2.35 1.05-3.11z"/></svg>
                App Store — próximamente
              </div>
            </div>
            <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.25)', marginTop: 20, marginBottom: 0 }}>También puedes suscribirte desde la app</p>
          </div>
        </div>
      )}
      ...
      <style jsx>{`
        ...
        .ring-big {
          ...
          border: 1px solid rgba(212,168,87,0.2); border-radius: 50%;
        ...
        .hero-sphere {
          ...
          border-radius: 50%;
          background: radial-gradient(circle at 30% 30%, rgba(212,184,150,0.3) 0%, rgba(212,168,87,0.35) 50%, rgba(184,201,168,0.25) 100%);
        ...
        /* FAQ accordion */
        .faq-item { border-bottom: 1px solid rgba(212,168,87,0.1); }
        .faq-question {
        ...
        .faq-question:hover { color: #D4A857; }
        .faq-answer {
        ...
        .btn-play {
          ...
          padding: 14px 28px; background: #D4A857; color: #1f2d3a;
        ...
        .btn-appstore-disabled {
          ...
          border: 1px solid rgba(212,168,87,0.15); border-radius: 14px;
